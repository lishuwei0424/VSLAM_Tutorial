...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
      ...
      //  if (!bInImage(u-8,v-8,image.cols,image.rows)||!bInImage(u+7,v+7,image.cols,image.rows))
      //       cout<<"关键点在边缘附近"<<kp.pt<<endl;

        if ((u-8<=image.cols&&v-8<=image.rows) ||(u+8<=image.cols&&v+8<=image.rows))
             cout<<"关键点在边缘附近"<<kp.pt<<endl;
        ...
        for(int i=-8;i<8;i++){
            for(int j=-8;j<8;j++){
                double gray=image.at<uchar>(v+j,u+i);
                m10+=(u+i)*gray;
                m01+=(v+j)*gray;
            }
        }

         kp.angle=atan2(m01,m10)*180/pi;
        // END YOUR CODE HERE
    }
    return;
}
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    ...
    for (auto &kp: keypoints) {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            qx += kp.pt.x;
            qy += kp.pt.y;
            if ((px<=image.cols&&py<=image.rows)||(qx<=image.cols&&qy<=image.rows))
            {
                d.clear();
                break;
            } else {
