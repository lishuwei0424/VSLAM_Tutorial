...
int main(int argc, char **argv) {
    ...
    Mat mDisparity = imread(file_disparity, 0);
    ...
    for (int i = 0; i < kp2_multi.size(); i++) {
        if (success_multi[i]) {
	    int iDisparity = mDisparity.at<uchar>(kp1[i].pt.y, kp1[i].pt.x);
	    int iOptFlowDisparity = kp1[i].pt.x - kp2_multi[i].pt.x;
	    int iDeltaDisp = iOptFlowDisparity - iDisparity;
//	    cout<<"Puv1: "<<kp1[i].pt << "Disparity: "<<iDisparity<<" OptFlowDisp: "<<iOptFlowDisparity<<
//		" iDeltaDisp: "<<iDeltaDisp << endl;
            vDispCompare.push_back(iDeltaDisp);
	    num_all++;
	    Scalar CirColor;
	    if(iDeltaDisp >= 5) {
                num0++;
		CirColor = Scalar(255,0,0);
	    } else if(iDeltaDisp < 5 && iDeltaDisp >= -5) {
		num1++;
		CirColor = Scalar(0,255,0);	
            } else if(iDeltaDisp < -5 && iDeltaDisp >= -20) {
  		num2++;
		CirColor = Scalar(0,255,255);
	    } else {
		num3++;
		CirColor = Scalar(0,0,255);
	    }
            cv::circle(img1_multi, kp1[i].pt, 2, CirColor, 2);
            cv::line(img1_multi, kp2_multi[i].pt, kp1[i].pt, cv::Scalar(0, 0, 250));
	    string text = "";
	    stringstream ss;
	    ss << iDeltaDisp;
	    ss >> text;
	    putText(img1_multi, text, kp1[i].pt, FONT_HERSHEY_COMPLEX, 0.5, CirColor, 1, 8, 0);
        }
    }
    cout<<"vDispCompare size: " << vDispCompare.size() << endl;
    cout<<"num_all: "<<num_all<<"\n >=5: "<<num0<<"\n [-5,5): "<<num1<<"\n [-20,-5): "<<num2<<"\n <-20: "<<num3<<endl;
    ...
    cv::imshow("tracked multi level", img1_multi);
