...
void BuildProblem(const BALProblem* bal_problem, g2o::SparseOptimizer* optimizer, const BundleParams& params)
{
	...
	VetexCamInput camin(temVecCamera);
        pCamera->setEstimate(camin);   // initial value for the camera i..
...
void WriteToBALProblem(BALProblem* bal_problem, g2o::SparseOptimizer* optimizer)
{
	...
	cout<<"1 WriteToBALProblem se3:"<<se3.transpose()<<endl;
	NewCameraVec << se3[3], se3[4], se3[5], se3[0], se3[1], se3[2], pCamera->estimate()._f, pCamera->estimate()._k1, pCamera->estimate()._k2;
	cout<<"2 WriteToBALProblem NewCameraVec: "<<NewCameraVec.transpose()<<endl;
